  ...
  begin
    ...
    if input_size_known and then output_size >= input_size then
      ...
      for I in 1 .. amount loop
         ...
         begin
            TempArray(1) := Stream_Element(OutBuf(I));
            Zip_Streams.Write (output.all, TempArray);
         end;
      end loop;
    ...
    if Source'Length < 1 then
      ...
      for I in Source'Range loop
        ...
        if feedback /= null then
          ...
          if TenPercent > 0 and then --  Bugfix GdM 23-Dec-2002
             ((Bytes_in-1) mod TenPercent = 0
...
begin
  ...
  begin
    ...
    if input_size_known then
      TenPercent := Integer(input_size / 10);
    else
      TenPercent := 0;
    end if;
