--
--  $Author: graham_s $
--  $Date: 2013-04-17 22:24:01 +0100 (Wed, 17 Apr 2013) $
--  $Revision: 16035 $
--
with Ada.Direct_IO;
...
package body frs_binary_reads is
   ...
   Prscrptn_File   : Prscrptn_io.File_Type;
   Nimigr_File     : Nimigr_IO.File_Type;
   ...
   begin
      ...
      if( current_year = 2003 or current_year = 2004 ) then
      ...
      if( current_year = 2003 or current_year = 2005 ) then
         Vehicle_io.Close (Vehicle_File);
      end if;
      Pension_io.Close (Pension_File);
      if( current_year < 2010 )then                                             
      ...
      if( current_year >= 2008 ) then                       
         Nimigr_io.Close( Nimigr_File );                         
      end if;
      if( current_year >= 2010 ) then                       
         Prscrptn_io.Close( Prscrptn_File );                         
      end if;
   ...
   begin
      ...
      if( year = 2003 or year = 2005 ) then
         Vehicle_io.Open                           
           (Vehicle_File,                          
            Vehicle_io.In_File,                    
      ...
      if( year >= 2008 ) then                       
         Nimigr_io.Open                         
           (Nimigr_File,                        
            Nimigr_io.In_File,                  
            BASE_DATA_DIR & String (DATA_YEAR_STRINGS (year)) & "/bin/nimigr.bin");
      end if;
      if( year >= 2010 ) then                       
         Prscrptn_io.Open                         
           (Prscrptn_File,                        
            Prscrptn_io.In_File,                  
            BASE_DATA_DIR & String (DATA_YEAR_STRINGS (year)) & "/bin/prscrptn.bin");
      end if;
      
   end Open_Files;

   function Bin_Read_Prscrptn (file : Prscrptn_io.File_Type; cpos : Positive) return Prscrptn_Rec is
      var : Prscrptn_Rec;
   begin
      Prscrptn_io.Read (file, var, Prscrptn_io.Count (cpos));
      return var;
   end Bin_Read_Prscrptn;

   function Bin_Read_Nimigr (file : Nimigr_io.File_Type; cpos : Positive) return Nimigr_Rec is
      var : Nimigr_Rec;
   begin
      Nimigr_io.Read (file, var, Nimigr_io.Count (cpos));
      return var;
   end Bin_Read_Nimigr;
   ...
   begin
      ...
      if (index.pointers (nimigrRec).startPos > 0) then
         hh.numNimigrs := index.pointers (nimigrRec).counter;
         for p in  1 .. index.pointers (nimigrRec).counter loop
            ptr             := index.pointers (nimigrRec).startPos + Integer (p) - 1;
            hh.nimigr (p) := Bin_Read_Nimigr (Nimigr_File, ptr);
            Assert
              (hh.nimigr (p).SERNUM = hh.household.SERNUM,
               "veh sernum mismatch " &
               hh.nimigr (p).SERNUM'Img &
               " hh " &
               hh.household.SERNUM'Img );
         end loop;
      end if;
      Ada.Text_IO.Put_Line( "Nimigr OK");
      ...
      -- FIXME these can apply to kids, too
      if( False )then
         if (index.pointers (PrscrptnRec).startPos > 0) then
            declare
               Prscrptns   : Prscrptn_Rec;
               person     : Integer;
               PrscrptnsNo : Pension_Count;
               adNo       : Integer;
            begin
               for p in  1 .. index.pointers (PrscrptnRec).counter loop
                  ptr      := index.pointers (PrscrptnRec).startPos + Integer (p) - 1;
                  Prscrptns := Bin_Read_Prscrptn (Prscrptn_File, ptr);
                  buno     := Integer (Prscrptns.BENUNIT);
                  person   := Integer (Prscrptns.PERSON);
                  --  this is safe since the jobs are always in ascending
                  --order
                  buno := personMapping (person).buno;
                  adNo := personMapping (person).adno;
                  Ada.Text_IO.Put_Line( "PrscrptnRec; buno " & buno'Img & "adno " & adno'Img );
                  hh.benunits (buno).adults (adNo).numPrscrptns           :=
                    hh.benunits (buno).adults (adNo).numPrscrptns + 1;
                  PrscrptnsNo                                             :=
                    Pension_Count (hh.benunits (buno).adults (adNo).numPrscrptns);
                  hh.benunits (buno).adults (adNo).Prscrptns (PrscrptnsNo) := Prscrptns;
                  Assert
                    (Prscrptns.SERNUM = hh.household.SERNUM,
                     "Prscrptns sernum mismatch");
               end loop;
            end;
         end if;
      end if;
