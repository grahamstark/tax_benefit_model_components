...
package body Zip.Headers is
  ...
  is
   ...
   begin
      for I in chb'Range loop
         declare
            TempArray : Stream_Element_Array (1..1);
            TempOffset : Stream_Element_Offset;
         begin
            Read (stream.all, TempArray, TempOffset);
            chb(I) := Zip.Byte (TempArray(1));
         end;
  ...
  begin
    ...
    chb(43..46):= Intel_bf( header.local_header_offset );
      ...
      declare
         TempArray : Stream_Element_Array ( 1 .. 1 );
      begin
         for I in chb'First .. chb'Last loop
            TempArray (1) := Stream_Element (chb(I));
            Write (stream.all, TempArray);
         end loop;
      end;
  end Write;
  ...
  is
   ...
   begin
      for I in lhb'Range loop
         declare
            TempArray : Stream_Element_Array (1..1);
            TempOffset : Stream_Element_Offset;
         begin
            Read (stream.all, TempArray, TempOffset);
            lhb(I) := Zip.Byte(TempArray(1));
         end;
      end loop;
   ...
   begin
      ...
      begin
         for I in lhb'Range loop
            TempArray (1) := Stream_Element (lhb(I));
            Write (stream.all, TempArray);
         end loop;
      end;
   end Write;
  ...
  begin
   ...
   begin
      declare
         ...
         TempOffset: Stream_Element_Offset;
      begin
         for I in eb'First .. eb'Last loop
            Read (stream.all, TempArray, TempOffset);
            eb(I) := Zip.Byte(TempArray(1));
         end loop;
  ...
  begin
    ...
    if Size(stream) <= 65_535 then
      min_end_start:= 1;
    else
      min_end_start:= Ada.Streams.Stream_IO.Count(Size(stream)) - 65_535;
    end if;
    ...
    for i in reverse min_end_start .. Ada.Streams.Stream_IO.Count(Size(stream)) - 21 loop
      ...
      begin
            for j in end_buffer'Range loop
               declare
                  strOffset: Stream_Element_Offset;
                  strArray : Stream_Element_Array (1..5);
               begin
                  Read(stream.all, strArray, strOffset);
                  end_buffer(j) := Unsigned_8(strArray(1));
               end;
  ...
  begin
    ...
    eb(21..22):= Intel_bf( the_end.main_comment_length );
      ...
      begin
         for I in eb'First .. eb'Last loop
            TempArray(1) := Stream_Element (eb(I));
            Write (stream.all, TempArray);
         end loop;
      end;
  end Write;
  ...
  is
   ...
   begin
      declare
         ...
         TempOffset : Stream_Element_Offset;
      begin
         for I in ddb'First .. ddb'Last loop
            Read (stream.all, TempArray, TempOffset);
            ddb(I) := Zip.Byte (TempArray(1));
         end loop;
      end;
  ...
  begin
    ...
    declare
       ...
       begin
         for I in ddb'First .. ddb'Last loop
            TempArray(1) := Stream_Element(ddb(I));
            Write (stream.all, TempArray);
         end loop;
      end;
  end Write;
