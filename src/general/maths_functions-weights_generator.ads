...
package Maths_Functions.Weights_Generator is
   ...
   subtype Col_Range is Positive range 1 .. Num_Constraints;
   subtype Row_Range is Positive range 1 .. Num_Observations;
   
   subtype Row_Vector is Vector( Col_Range );
   subtype Col_Vector is Vector( Row_Range );
   
   subtype Dataset is Matrix( Row_Range, Col_Range );
   ...
   function Sum_Dataset( 
      data    : Dataset;
      weights : Col_Vector ) return Row_Vector;
      
private
      
   subtype Hessian_Matrix is Matrix( Col_Range, Col_Range );
