...
def parseOneYear( connection, filename , yearStr, datapath )
        ...
        table = ''
        variable = ''
        insert_enum_stmt = connection.prepare( "insert into enum(year, tables, variable, value, label ) VALUES (?, ?, ?, ?, ? )" )
        insert_tables_stmt = connection.prepare( "insert into tables(year, name) VALUES (?, ?)" )
...
def loadTable( connection, tableName, year = nil )
        ...
        rs.fetch_hash{
                ...
                enumStmt = "select value, label from enum where tables='#{tableName}' and variable = '#{var.name}' and year='#{year}' order by value"
                ers = connection.execute( enumStmt )
                ers.fetch_hash{
                        |eres|
                        enum = Enum.new
                        enum.value = eres['fssvalue']
                        enum.label = eres['label']
                        var.enums << enum
                }
