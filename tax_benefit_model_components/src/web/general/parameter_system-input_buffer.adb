...
package body Parameter_System.Input_Buffer is
   ...
   begin
      -- Put_Line( " Year_Prefix_Str " & buff.Has_Years'Img & "first_possible_year " & buff.first_possible_year'Img &
      ...
      case param_and_value.etype is
         ...
         when map_of_arrays => 
            declare
               vel : Value_And_Error_Vector;
            begin
               Put_Line( "Get_Value_And_Error: looking for postfix = " & TS( postfix ) & " index = " & index'Img );
   ...
   begin
      ...
      Put_Line( "searching for key |" & TS( lkey ) & "| buff.Year_Prefix_Str = " & TS( buff.Year_Prefix_Str ));
      ...
      if( ve /= Null )then
         ...
         Put_Line( "|" & TS( key ) & "| is not an integer" ); 
      else
         Put_Line( "nothing found for key " & TS( key ) & " keys are " & Print_Keys( buff.params ));
      end if;
      return 0;
   end Get;
   ...
   begin
      ...
      Put_Line( "Get (UBS) searching for key |" & TS( lkey ) & "| buff.Year_Prefix_Str = " & TS( buff.Year_Prefix_Str ));
      ve := buff.Get_Value_And_Error( lkey, index, postfix );   
      if( ve /= Null )then
         ...
         Put_Line( "Get string nothing found for key " & TS( key ) & " keys are " & Print_Keys( buff.params ));
      end if;
      return Null_Unbounded_String;
   end Get;
                     ...
                     begin
                        ...
                        for pno in 1 .. num_params loop
                           ...
                           for i in 1 .. counter loop
                              ...
                              Log( "looking for index_key " & To_String( index_key ));
                              Put_Line( "looking for index_key " & To_String( index_key ));
                              default_value := defaults.Element( index_key );
                              value_str := values.Element( index_key );
                              Log( "found value " & To_String( default_value ));
                              value := Create_Value_And_Error_Access( 
   ...
   function Init( 
      ...
      values      : Keyed_Text_Buffer.Text_Buffer;
      defaults    : Keyed_Text_Buffer.Text_Buffer;
      start_yr    : Year_Number := Year_Number'First;
      end_yr      : Year_Number := Year_Number'First;
      year_prf    : Unbounded_String := Null_Unbounded_String ) return Buffer is
      buff : Buffer;
   ...
   begin
      ...
      buff.first_possible_year := start_yr;
      buff.last_possible_year := end_yr;
      buff.year_prefix := year_prf;
      if( buff.Has_Years )then
         for year in buff.first_possible_year .. buff.last_possible_year loop
            path := buff.Year_Prefix_Str( year );
            Load( buff,
                  sys,
                  sys,
                  values,
