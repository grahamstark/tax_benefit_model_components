...
package body Format_utils is
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "FORMAT_UTILS" );
  ...
  begin
   ...
   function Format( r : Float_Type; prec : Natural := 2 ) return String is
      s : String( 1 .. 100 ) := ( others => ' ' );
      lr : Float_Type := Float_Type'Rounding( r * 100.0 );
      p  : Natural := 1;
   begin
      
      FIO.Put( s, r, Text_IO.Field( prec ), 0 );
      return Text_Utils.Delete_All_Instances( s, " " );
   ...
   function Format_L( r : Float_Type; prec : Natural := 2 ) return Unbounded_String is
   begin
      return TuS( Format( r, prec ));
   end Format_L;
