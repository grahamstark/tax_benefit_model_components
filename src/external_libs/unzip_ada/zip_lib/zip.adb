  ...
  begin -- Load Zip_info
    ...
    Zip_Streams.Set_Index(
      ...
      for i in 1..the_end.total_entries loop
         declare
            tempStream : constant Zip_Streams.Zipstream_Class := from;
         begin
      ...
      begin
         for k in this_name'Range loop
           ...
           begin
              Zip_Streams.Read(from.all, strArray, strOffset);
              b := Unsigned_8(strArray(1));
           end;
    ...
    Binary_tree_rebalancing.Rebalance(p);
    info:= ( loaded           => True,
             zip_file_name    => new String'("Not from file, but stream"),
    ...
    Zip.Headers.Load(file, the_end);
      ...
      begin
            for k in this_name'Range loop
               ...
               begin
                  Read (file.all, TempArray, TempOffset);
                  b := Unsigned_8(TempArray(1));
               end;
   ...
   begin
      if End_Of_Stream(file) then
        actually_read := 0;
         ...
         actually_read:=
         Integer'Min( buffer'Length, Integer(Size(file) - Index(file) + 1) );
         for I in buffer'First .. buffer'First + actually_read - 1 loop
            declare
               strOffset: Stream_Element_Offset;
               strArray : Stream_Element_Array (1..5);
            begin
               Read(file.all, strArray, strOffset);
               buffer(I) := byte(strArray(1));
