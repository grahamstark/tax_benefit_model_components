...
procedure UnZipAda is
  ...
  fda:          feedback_proc         := My_feedback'Access;
  ...
  begin
     ...
     Put_Line("Demo for the UnZip-Ada library, by G. de Montmollin");
...
begin
  if Argument_Count=0 then
    ...
    if Argument_Count = last_option then -- options only ?!
      ...
      begin
        if not Ada.Directories.Exists(Archive) then
          zip_ext:= True;
          if not Ada.Directories.Exists(Archive) then
