...
package body Parameter_System.XML is
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "PARAMETER_SYSTEM.XML" );
   ...
   begin
      ...
      for dno in 1 .. dnodes.Length( descriptions ) loop
         ...
         -- Log( "#4 " & dno'Img & " name " & dnodes.Node_Name( ds ));
         -- Log( "#4A " );
         -- if dnodes.Node_Name( ds ) = "Description" then
         --   Log( "#5aaa " & dno'Img );
         -- end if;
         -- Log( "dnodes.Node_Name( ds ) = |" & dnodes.Node_Name( ds ) & "|" );
         if dnodes.Node_Name( ds ) = "Description" then
            -- Log( "Got description node" );
   ...
   begin
      ...
      for dno in 1 .. dnodes.Length( ei_elems ) loop
         ...
         -- Log( "got onchange as " & TS( ei.onchange ));
      end loop;
      return ei;
   end Load_Edit_Info;
   ...
   begin
      ...
      -- Log( "pr.instance_name=|" & TS( pr.instance_name ) & "|" );
