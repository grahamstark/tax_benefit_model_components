...
package body UnZip is
    ...
    if name_from_header then
      ...
      begin
         for i in 1..the_name_len loop
               ...
               begin
                  Read (zip_file.all, TempArray, TempOffset);
  ...
  procedure Extract( from                 : String;
   ...
   is
    use Zip_Streams;
    MyStream     : aliased Unbounded_Stream;
  ...
  begin
    if feedback = null then
      ...
      -- Copy the content of the streams
      declare
         TempArray : Stream_Element_Array (1..1);
         TempOffset : Stream_Element_Offset;
      begin
         while not End_Of_File(file) loop
            Read (file, TempArray, TempOffset);
            Write (zip_file.all, TempArray);
         end loop;
      end;
  ...
  exception
    when Zip.Headers.bad_local_header =>
      Close(file); -- normal case: end was hit
